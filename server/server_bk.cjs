...
initializeGoogleClients().then(async () => {
    ...
    // --- REVISED: /api/order-stats endpoint for robust date range counting ---
        ...
        try {
            ...
            // Initialize count object with all dates set to 0
            const countByDate = {};
    ...
    // --- New endpoint for popular items ---
    ...
    // --- New endpoint for hourly order counts ---
    app.get('/api/hourly-orders', async (req, res) => {
        try {
            ...
            // Initialize hourly counts with zeros
            const hourlyCounts = {};
    ...
    app.post('/api/fire-order', express.json(), async (req, res) => {
      ...
      try {
        const result = await sendToPrinter(order);
        res.json({ success: true, printerResponse: result });
      } catch (err) {
        res.status(500).json({ error: 'Failed to print order', details: err.message });
      }
    });
